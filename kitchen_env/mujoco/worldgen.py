...
from ..constants import OBS_ELEMENT_INDICES, OBS_ELEMENT_GOALS, FRANKA_INIT_QPOS
...
class MujocoWorldgenKitchenEnvWrapper(gym.Wrapper):
    ...
    def _change_cabinet_texture(self, cabinet, cabinet_texture):
        if cabinet_texture not in ['wood1', 'wood2', 'metal1', 'metal2', 'marble1', 'tile1']:
    ...
    def change_objects_layout(self, object, layout):
        LAYOUTS = {
            ...
            'ovenhood': {'right_raised': dict(pos="0.1 0 0.35")},
        }
        ...
        if object == 'kettle':
            ...
            if kettle_qpos is not None:
                self.changed['init_qpos'][OBS_ELEMENT_INDICES['kettle']] = kettle_qpos
            if kettle_qrot is not None:
                self.changed['init_qpos'][OBS_ELEMENT_INDICES['kettle_rot']] = kettle_qrot
        else:
            o = object
    ...
    def reset_domain_changes(self):
        ...
        self.domain_model_xml_tree = ET.fromstring(self.model_xml, parser=self.parser)
    ...
    def reset(self, reload_model_xml=False, **kwargs):
        ...
        objects_done_set = self.objects_done_set if len(self.objects_done_set) > 0 else None
